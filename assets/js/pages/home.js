...
const testimonal = document.querySelector(".testimonal-container");

const observer = new IntersectionObserver(handleObserve, { threshold: 0.4 });

function handleObserve(entries) {
  entries.forEach((entry) => {
    if (entry.isIntersecting) {
      testimonal.classList.add("active");
    } else {
      testimonal.classList.remove("active");
    }
  });
}
observer.observe(testimonal);
